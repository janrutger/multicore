...
class ZScriptExpander(Transformer):
    def __init__(self):
        super().__init__()
        self.macro_table = {}
        self.symbol_table = {}

        # Standaard geheugensegmenten conform STERN-Z32
        self.memsize = 2560
        self.program_size = 1024
        self.shared_size = 1024
        self.private_size = 512
        self.sp_size = 0  # Default STACK_SIZE = 0
        self.explicit_memory_block = False

        # Geheugen-pointers en grenzen
        self.allocator_initialized = False

        # Shared RAM (Groeit OPWAARTS)
        self.shared_start = None
        self.shared_limit = None
        self.next_shared_address = None

        # Private RAM (Groeit NEERWAARTS onder de Stack)
        self.private_start = None
        self.private_limit = None
        self.next_private_address = None
    ...
    def _initialize_allocator(self):
        if not self.allocator_initialized:
            # 1. Controleer of een expliciet MEMORY-blok gebruikt is
            if self.explicit_memory_block:
                if (
                    self.program_size is None
                    or self.shared_size is None
                    or self.private_size is None
                ):
                    raise SyntaxError(
                        "[ZScript Compiler Error] Onvolledige MEMORY map! "
                        "PROGRAM, SHARED en PRIVATE moeten alle drie worden gedefinieerd."
                    )
                self.memsize = (
                    self.program_size + self.shared_size + self.private_size
                )
            else:
                # Fallback voor legacy scripts
                if self.memsize == 2560:
                    self.program_size = 1024
                    self.shared_size = 1024
                    self.private_size = 512
                else:
                    self.program_size = self.memsize // 2
                    self.shared_size = self.memsize // 4
                    self.private_size = self.memsize // 4

            # 2. SHARED RAM: Groeit OPWAARTS vanaf program_size
            self.shared_start = self.program_size
            self.shared_limit = self.shared_start + self.shared_size - 1
            self.next_shared_address = self.shared_start

            # 3. PRIVATE RAM: Groeit NEERWAARTS vanaf de top (onder de SP stack)
            self.private_start = self.shared_start + self.shared_size
            self.private_limit = (
                self.private_start + self.private_size - 1
            )  # Bijv. 2559
            # Start direct onder de gereserveerde stack:
            self.next_private_address = self.private_limit - self.sp_size

            self.allocator_initialized = True

            print(
                f"[ZScript] Geheugen geconfigureerd: MEMSIZE={self.memsize} "
                f"(ROM={self.program_size}, SHARED={self.shared_size}, "
                f"PRIVATE={self.private_size}), STACK_SIZE={self.sp_size}"
            )
            print(
                f"[ZScript] Shared RAM (SRES) : Adres {self.shared_start} t/m"
                f" {self.shared_limit} (groeit omhoog)"
            )
            print(
                f"[ZScript] Private RAM (PRES): Start op adres"
                f" {self.next_private_address} (groeit omlaag richting"
    ...
    def program_size_stmt(self, items):
        self.program_size = int(items[0])
        self.explicit_memory_block = True
        return None

    def shared_size_stmt(self, items):
        self.shared_size = int(items[0])
        self.explicit_memory_block = True
        return None

    def private_size_stmt(self, items):
        self.private_size = int(items[0])
        self.explicit_memory_block = True
        return None
    ...
    def sres_stmt(self, items):
        """SRES: Wijs toe in SHARED RAM (opwaarts)."""
        self._initialize_allocator()
        name = str(items[0])
        size = int(items[1]) if len(items) > 1 else 1

        # Check opwaartse overflow
        if self.next_shared_address + size - 1 > self.shared_limit:
            raise SyntaxError(
                f"[ZScript Compiler Error] Shared RAM Overflow! "
                f"Kan '{name}' ({size} words) niet toewijzen. "
                f"Shared RAM limiet ({self.shared_limit}) overschreden."
            )

        start_addr = self.next_shared_address
        ...
        self.next_shared_address += size

        print(
            f"[ZScript Allocator] SRES '{name}' (grootte {size}) -> Shared RAM:"
            f" Adres {start_addr} t/m {end_addr}"
        )
        return None

    def pres_stmt(self, items):
        """PRES: Wijs toe in PRIVATE RAM (neerwaarts onder de stack)."""
        self._initialize_allocator()
        name = str(items[0])
        size = int(items[1]) if len(items) > 1 else 1

        # Bereken het laagste (start) adres van dit blok
        assigned_address = self.next_private_address - size + 1

        # Check neerwaartse underflow (mag niet onder private_start komen)
        if assigned_address < self.private_start:
            raise SyntaxError(
                f"[ZScript Compiler Error] Private RAM Overflow! "
                f"Kan '{name}' ({size} words) niet toewijzen. "
                f"Private RAM bodem ({self.private_start}) overschreden."
            )

        self.symbol_table[name] = {
            "type": "RES",
            "value": assigned_address,
            "size": size,
            "segment": "PRIVATE",
        }
        self.next_private_address -= size

        print(
            f"[ZScript Allocator] PRES '{name}' (grootte {size}) -> Private RAM:"
            f" Adres {assigned_address} t/m {assigned_address + size - 1}"
        )
        return None

    def res_stmt(self, items):
        """Achterwaarts compatibel: Generieke RES wijst neerwaarts toe in PRIVATE RAM."""
        return self.pres_stmt(items)
