MAP {
    ...
    ; === GEHEUGEN ALLOCATIE ===
    SHARED cur_pixel 1         ; Volgende pixel-index die uitgedeeld moet worden (Adres 2047)
    SHARED active_count 1      ; Aantal actieve workers op dit moment (Adres 2046)

    ; === 12 MAILBOX SLOTS IN SHARED RAM ===
    SHARED mb_px 12            ; Globale X-coördinaat per slot (Adres 2034..2045)
    SHARED mb_py 12            ; Globale Y-coördinaat per slot (Adres 2022..2033)
    SHARED mb_color 12         ; Berekend iteratie-resultaat per slot (Adres 2010..2021)
    SHARED mb_status 12        ; Status vlag per slot: 0=EMPTY, 1=BUSY, 2=DONE (Adres 1998..2009)

    ; === CANVAS & MAILBOX CONFIGURATIE ===
    CONST IMG_WIDTH 200      ; Totale canvas breedte
    CONST IMG_HEIGHT 200     ; Totale canvas hoogte
    CONST TOTAL_PIXELS 40000 ; 200 x 200 = 40000 pixels totaal
